...
res_sum = df_res_all.groupby(["商品名", "サイズ", "地名"])["數量"].sum().reset_index().rename(columns={"数量": "予約計"}) if not df_res_all.empty else pd.DataFrame(columns=["商品名", "サイズ", "地名", "予約計"])
                        ...
                        for idx, p in update_payload.items():
                            ...
                            if p["delete"]:
                                ...
                                if p["type"] == "予約出庫":
                                    ...
                                    new_logs.append({"日時": now, "商品名": row["商品名"], "サイズ": row["サイズ"], "地名": p["loc"], "区分": p["type"], "數量": p["qty"], "在庫数": new_df_stock.at[target_idx, "在庫数"], "担当者": user_name})
